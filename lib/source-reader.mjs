...
async function fetchPage(url, { signal } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const req = requester(url, {
      signal, method: "GET", timeout: 8000, autoSelectFamily: false, headers: { "user-agent": "ReaderTranslationLibrary/1.5", accept: "text/html,text/plain" },
...
export async function readPublicSource(value, { requestImpl = fetchPage, term = "", signal } = {}) {
  let url = checkedUrl(value);
  for (let redirects = 0; redirects <= 2; redirects++) {
    signal?.throwIfAborted();
    const response = await requestImpl(url, { signal });
