...
export async function searchResearchSources({ query, context = "", apiKey, budget, signal, fetchImpl = fetch }) {
  ...
  return budget.run(searchText, async () => {
    ...
    let response;
    try { response = await fetchImpl(endpoint, { headers: { "X-Subscription-Token": apiKey, accept: "application/json" }, signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(8000)]) : AbortSignal.timeout(8000) }); }
...
export async function verifyIssue({ book, item, kind, apiKey, budget, provider, signal, requestsPerItem = 2, search = searchResearchSources, readPage = readPublicSource, analyze = researchTranslationIssue }) {
  ...
  for (const extra of queries.slice(0, Math.max(0, Math.min(2, requestsPerItem)))) {
    signal?.throwIfAborted(); let found;
    try { found = await search({ query: originalTerm, context: extra, apiKey, budget, signal }); }
    ...
    const opened = await Promise.allSettled(found.slice(0, 3).map((entry) => readPage(entry.url, { term: originalTerm, signal })));
    ...
    try {
      const result = await analyze({ provider, book, item, kind, evidence, signal });
