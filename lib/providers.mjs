export function assertFinished(result) {
  if (!["stop", "completed", "end_turn"].includes(result.finishReason)) {
    const error = new Error(`模型输出未完整结束（${result.finishReason || "unknown"}）；此块未采用，可从此块重试`);
...
export async function generate({ provider, messages, responseSchema, signal, onEvent }) {
  ...
  if (!response.ok) throw new Error(value.error?.message || value.message || `API 请求失败 (${response.status})`);
  ...
  const result = assertFinished({ text: text.trim(), finishReason: responses ? value.status : value.choices?.[0]?.finish_reason,
