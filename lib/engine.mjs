...
import { alignmentSchema, parseAlignedText, sourceParagraphs, translationBlocks, revisionSegments } from "./alignment.mjs";
...
export async function translateChapter({ provider, book, chapter, source, paragraphs = sourceParagraphs(source, chapter.id), previousTail = "", glossary = [], characters = [], existingDraft = "", draftSegments = revisionSegments(chapter), mode = "draft", correction = null, control = {}, onProgress, onBlock, resumeBlocks = [] }) {
  ...
  if (legacy && source.length > 6500) throw new Error("旧译文没有段落映射；请保留此版本并重新初译以建立对齐后再精校");
  const pieces = legacy ? [{ id: "legacy-chapter", order: 0, paragraphs, sourceParagraphIds: paragraphs.map((p) => p.id), draft: existingDraft }] : translationBlocks(paragraphs, mode === "refine" ? draftSegments : null);
  ...
  for (const piece of pieces) {
    ...
    if (!block) {
      ...
      try {
        ...
        if (!control.cancelled && !control.signal?.aborted) await onBlock?.({ id: piece.id, order: piece.order, sourceParagraphIds: piece.sourceParagraphIds, status: "failed", error: error.message, partialText: error.partialText || "", finishReason: error.finishReason });
        throw error;
      }
    }
    outputs.push(block);
