...
const mock = http.createServer(async (req, res) => {
  ...
  if (req.url.endsWith("/responses")) res.end(JSON.stringify({ output_text: "这是 Responses 接口译文。", usage: { input_tokens: 80, output_tokens: 20 } }));
  else if (requestBody.includes("remainingQuestion")) res.end(JSON.stringify({ choices: [{ message: { content: JSON.stringify({ suggestedChinese: "洛中", definition: "京都城内", translatorNote: "此处指京都城内。", reason: "来源说明了所指", remainingQuestion: "", verdict: "supported", confidence: "medium", sourceUrls: ["https://www.wikidata.org/wiki/Q1", "https://evil.example/"] }) } }], usage: { prompt_tokens: 60, completion_tokens: 30 } }));
  else if (requestBody.includes("术语、人名与疑难项")) res.end(JSON.stringify({ choices: [{ message: { content: JSON.stringify({ terms: [{ japanese: "山口組", reading: "やまぐちぐみ", chinese: "山口组", category: "组织", confidence: "high", note: "固定组织名" }], characters: [], uncertainties: [] }) } }], usage: { prompt_tokens: 90, completion_tokens: 30 } }));
  else res.end(JSON.stringify({ choices: [{ message: { content: "这是第一段译文。\n\n这是第二段译文。" } }], usage: { prompt_tokens: 120, completion_tokens: 40 } }));
});
